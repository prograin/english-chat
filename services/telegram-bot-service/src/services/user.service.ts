...
export const createUserService = async (data: object) => {
  ...
  if ([201, 409].includes(user.status)) {
    await UsersCache.setUserIdByTelegramId(
      user.data.userTelegramId,
      user.data.id
    );
    return user;
  } else {
    throw new Error(
      `❌ Unexpected response status when creating user: ${user.status}`
    );
  }
};
...
export const getUserByTelegramIdService = async (telegram_id: bigint) => {
  ...
  if (userId) {
    ...
    if (user.status === 200) {
      ...
      UsersCache.deleteUserByTelegramId(telegram_id);
      throw new Error(
        `❌ Unexpected response status when fetching user: ${user.status}`
      );
    }
  } else {
    // --- 2. Try to get by telegram id ---
    user = await axios.get(
      `http://localhost:3000/users/?telegram_id=${telegram_id}`,
      { validateStatus: (status) => status < 500 }
    );
