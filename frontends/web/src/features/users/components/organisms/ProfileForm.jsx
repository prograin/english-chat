...
export default function ProfileForm({ user, onChange, onSave, onDiscard, onChangeInterests, onChangeLocation, saving, isDirty, options }) {
  return (
    <form className="space-y-md font-sans">
      ...
      <FieldRow>
        <ProfileField field={{ ...PROFILE_FIELDS[6], options: options.countriesOption }} value={user.country} onChange={onChangeLocation} />
        <ProfileField field={{ ...PROFILE_FIELDS[7], options: options.statesOption }} value={user.state} onChange={onChangeLocation} />
        <ProfileField field={{ ...PROFILE_FIELDS[8], options: options.citiesOption }} value={user.city} onChange={onChangeLocation} />
      </FieldRow>

      <ProfileField field={PROFILE_FIELDS[9]} value={user.description} onChange={onChange} />

      <div className="flex gap-sm mt-md">
        <SaveButton onClick={onSave} disabled={!isDirty || saving} saving={saving} className="button-glass" />
